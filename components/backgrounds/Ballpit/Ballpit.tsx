...
class W {
  ...
  setSizesAndShapes() {
    ...
    sizeData[0] = config.size0;
    {
      ...
      scale3Visual.set(
        [sx * config.size0, sy * config.size0, sz * config.size0],
        0
      );
    }
    ...
    for (let idx = startIdx; idx < c.count; idx++) {
      ...
      if (c.controlSphere0) {
        const d0 = new Vector3()
          .copy(new Vector3().fromArray(this.positionData, 0))
          .sub(pos);
        const d = d0.length();
...
class Z extends InstancedMesh {
  ...
  constructor(renderer: WebGLRenderer, params: Partial<typeof DEFAULTS> = {}) {
    ...
    geometry.setAttribute(
      "aRand",
      new InstancedBufferAttribute(seeds, 3, false)
    );
  ...
  #setupLights() {
    ...
    this.dirLight = new DirectionalLight(
      this.config.dirLightColor as any,
      this.config.dirLightIntensity
    );
...
function createBallpit(
  ...
  function createGround() {
    ...
    const mat = new ShadowMaterial({
      opacity: 0.25,
      color: new Color(0x000000) as any,
    });
...
const BallpitChips: React.FC<BallpitProps> = ({
  className = "",
  followCursor = true,
  ...props
}) => {
